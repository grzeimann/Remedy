...
def fit_template(records, coefficients, residuals):
    if not residuals:
        return np.full(N_WAVE, np.nan)
    y = np.asarray(residuals, dtype=float)
    x = np.asarray([coefficients[record["hardware"]] for record in records], dtype=float)
    counts = np.clip(np.asarray([record["n"] for record in records], dtype=float), 1.0, 112.0)
...
def fit_coefficients(records, templates, residuals, keys):
    answer = {key: 0.0 for key in keys}
    for key in keys:
        selected = [(record, residual) for record, residual in zip(records, residuals)
        ...
        for record, residual in selected:
            ...
            y.extend(residual[use]); x.extend(template[use]); n.extend([record["n"]] * int(np.sum(use)))
        answer[key] = robust_slope(y, x, n)
    return answer
...
def solve_additive(exposure_data, ifu_obs, amp_obs, ifu_keys, amp_keys, max_iterations, tolerance):
    ...
    R = {e: robust_spectrum(np.asarray([rec["y"] for (ee, _), rec in ifu_obs.items() if ee == e])) for e in exposures}
    c = {key: 0.0 for key in ifu_keys}; d = {key: 0.0 for key in amp_keys}
    U = {e: max((rec["y"] - R[e] for (ee, _), rec in ifu_obs.items() if ee == e), key=robust_rms, default=np.zeros(N_WAVE)).copy() for e in exposures}
    residuals = [rec["y"] - R[e] for (e, _), rec in ifu_obs.items()]
    c = fit_coefficients(ifu_records, U, residuals, ifu_keys)
    c, U, R, c_norm = normalize_ifu(c, U, R, supported_c)
    V = {}
    for e in exposures:
        amp_start = [rec["y"] - R[e] - c[key[:3]] * U[e]
                     for (ee, key), rec in amp_obs.items() if ee == e]
        V[e] = max(amp_start, key=robust_rms, default=np.zeros(N_WAVE)).copy()
    history = []
    previous_prediction = None
    for iteration in range(1, max_iterations + 1):
        ifu_residuals = [rec["y"] - R[e] for (e, _), rec in ifu_obs.items()]
        c = fit_coefficients(ifu_records, U, ifu_residuals, ifu_keys)
        for e in exposures:
            selected = [res for record, res in zip(ifu_records, ifu_residuals) if record["exposure"] == e]
            records = [record for record in ifu_records if record["exposure"] == e]
            if selected:
                U[e] = fit_template(records, c, selected)
        c, U, R, c_norm = normalize_ifu(c, U, R, supported_c)
        amp_residuals = [rec["y"] - R[e] - c[key[:3]] * U[e] for (e, key), rec in amp_obs.items()]
        d = fit_coefficients(amp_records, V, amp_residuals, amp_keys)
        for e in exposures:
            selected = [res for (ee, _), res in zip(amp_obs, amp_residuals) if ee == e]
            records = [record for record in amp_records if record["exposure"] == e]
            if selected:
                V[e] = fit_template(records, d, selected)
        d, V, d_norm = normalize_amp(d, V, ifu_keys, amp_keys, supported_d)
        for e in exposures:
            values = [rec["y"] - c[key[:3]] * U[e] - d[key] * V[e] for (ee, key), rec in amp_obs.items() if ee == e]
            if values:
                R[e] = robust_spectrum(np.asarray(values))
        prediction = np.asarray([R[e] + c[key[:3]] * U[e] + d[key] * V[e] for (e, key) in amp_obs])
        ...
        stage3 = np.asarray([rec["y"] - prediction[index] for index, ((e, key), rec) in enumerate(amp_obs.items())])
        raw_change = float(np.nanmax(np.abs(prediction)))
        history.append({"iteration": iteration, "invariant_prediction_change": delta,
                        "invariant_prediction_change_rms": change_rms,
                        "raw_prediction_scale": raw_change, "stage3_robust_rms": robust_rms(stage3),
...
def synthetic_test():
    ...
    R, U, V, c, d, history, converged = solve_additive(exp_data, ifu_obs, amp_obs, all_ifus, all_amps, 20, 1e-7)
...
def write_product(path, wavelength, exp_rows, ifu_rows, amp_rows, obs_rows, gamma, iota, eta, gamma_sigma, iota_sigma, eta_sigma, metadata, overwrite=False):
    ...
    class Metadata(tables.IsDescription): key=tables.StringCol(128); value=tables.StringCol(12000)
    ...
    with tables.open_file(path, "w", title="M101 clean calibration") as h5:
        h5.root._v_attrs.schema_version=SCHEMA; h5.create_array("/wavelength", "lambda", np.asarray(wavelength,float)); h5.create_group("/", "provenance"); h5.create_group("/", "exposures"); h5.create_group("/", "physical_ifus"); h5.create_group("/", "physical_amplifiers"); h5.create_group("/", "amplifier_observations"); h5.create_group("/", "global_parameters")
...
def main():
    ...
    started=time.perf_counter(); h5_paths=discover_h5(args); blank,blank_id=load_blank(args.external_blank_fibers,h5_paths); filters={"ON":read_filter(args.on_filter),"OFF":read_filter(args.off_filter)}; filter_ids={key:identity(value) for key,value in (("ON",args.on_filter),("OFF",args.off_filter))}
    exp_data,ifu_obs,amp_obs,ifu_keys,amp_keys=collect_blank_groups(h5_paths,blank,args.minimum_finite_fraction,args.min_blank_ifus,args.min_blank_per_amp); R,U,V,c,d,history,converged=solve_additive(exp_data,ifu_obs,amp_obs,ifu_keys,amp_keys,args.max_additive_iterations,args.additive_tolerance)
    if not converged: raise RuntimeError("additive invariant prediction did not converge: %s" % history[-1])
    band=collapse_components(R,U,V,filters); rows,ids,measurement_meta=load_measurement_h5(args.measurements); compact=apply_compact_mask(rows,args.compact_mask); h5_name={int(k):v for k,v in ids.items()}
    ...
    gamma,iota,eta,gamma_sigma,iota_sigma,eta_sigma,global_qa=build_hierarchy(obs)
    ...
    for e in sorted(exp_data):
        terms=exp_data[e]; exp_rows.append({"H5":e[0],"exposure":e[1],"R_lambda":R[e],"U_lambda":U[e],"V_lambda":V[e],"R_ON_OFF":band[(e,"R")],"U_ON_OFF":band[(e,"U")],"V_ON_OFF":band[(e,"V")],"R_null":[weighted_scalar(R[e],x) for x in nulls],"U_null":[weighted_scalar(U[e],x) for x in nulls],"V_null":[weighted_scalar(V[e],x) for x in nulls],"iterations":len(history),"converged":True,"invariant_prediction_change":history[-1]["invariant_prediction_change"],"invariant_prediction_change_rms":history[-1]["invariant_prediction_change_rms"],"n_blank_fibers":terms["n_blank"],"n_blank_ifus":terms["n_ifu"],"n_blank_amplifiers":terms["n_amp"]})
    ifu_rows=[]
    for key in ifu_keys:
    ...
    metadata={"schema_version":SCHEMA,"created_utc":time.strftime("%Y-%m-%dT%H:%M:%SZ",time.gmtime()),"script":str(Path(__file__).resolve()),"measurement":identity(args.measurements),"input_h5":[identity(p) for p in h5_paths],"external_blank_fibers":blank_id,"filters":filter_ids,"measurement_metadata":measurement_meta,"model_equation":"W = exp(z) S + R + cU + dV + noise","native_final_equation":"[Fibers.spectrum/Survey.offset - R - cU - dV] / exp(posterior_z_mean)","uses_Fibers_skyspectrum":False,"uses_sky_blank_for_z":False,"fits_p":False,"fits_alpha":False,"fits_q_mode":False,"source_likelihood":"external_valid + compact_mask + date/finite/error; |X|<=1e-12 has zero z information","delta_z_band":0.0,"error_model":"sqrt((s_b*sigma_data)^2 + (f_b*X)^2)","normalization":{"c":"robust location zero, robust scale one; amplitude in U","d":"arithmetic centered within physical IFU, pooled robust scale one; amplitude in V"},"convergence":{"history":history,"iterations":len(history),"converged":True,"invariant_prediction_change":history[-1]["invariant_prediction_change"]},"runtime_seconds":time.perf_counter()-started,"unsupported_ifus":[list(k) for k in ifu_keys if k not in {x[1] for x in ifu_obs}],"unsupported_amplifiers":[list(k) for k in amp_keys if k not in {x[1] for x in amp_obs}]}
    if output.exists() and not args.overwrite and not args.resume: raise SystemExit("output exists; use --overwrite: %s" % output)
    write_product(output,WAVE,exp_rows,ifu_rows,amp_rows,obs_rows,gamma,iota,eta,gamma_sigma,iota_sigma,eta_sigma,metadata,overwrite=args.overwrite); print(json.dumps({"output":str(output),"runtime_seconds":time.perf_counter()-started,"iterations":len(history),"convergence_metric":history[-1]["invariant_prediction_change"],"supported_ifus":sum(x["supported"] for x in ifu_rows),"unsupported_ifus":[x for x in ifu_rows if not x["supported"]],"supported_amplifiers":sum(x["supported"] for x in amp_rows),"unsupported_amplifiers":[x for x in amp_rows if not x["supported"]],"amplifier_observations":len(obs_rows),"delta_z_band":0.0},indent=2,default=str))
